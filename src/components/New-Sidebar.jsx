...
import { Accordion, AccordionButton, AccordionIcon, AccordionItem, AccordionPanel, Badge, Box, Flex, HStack, Image, Text, VStack } from "@chakra-ui/react";
...
//Main
const Sidebar = ({ notification, fetchNotification, notificationWithParams, fetchNotificationWithParams }) => {
  return (
    <Flex h="100vh" width="15rem" bg="primary" color="#D1D2D5" justifyContent="space-between" flexDirection="column">
...
//Navigation
const SidebarList = ({ notification, fetchNotification, notificationWithParams }) => {
  ...
  const sideBars = [
    ...
    },
    {
      ...
      notifcation: notificationWithParams?.returnedApproved?.returnedApprovecount,
    },
    {
    ...
    },
    {
      ...
      notifcation: notification?.forApprovalMoveOrderAll?.forapprovalmoveorderallcount,
    },
  ];
      ...
      <PageScroll minHeight="auto" maxHeight="550px">
        <Accordion allowToggle w="full">
          {tagModules?.map((sidebarMenu, i) => (
            <AccordionItem key={i} border="none" fontWeight="semibold" _focus={{ outline: "none" }}>
              <AccordionButton
                w="full"
                onClick={() => {
                  ...
                  {menu?.map((sub, j) => (
                    <Link to={sub.path} key={sub.path}>
                      <HStack
                        ...
                        p={2}
                        fontSize="xs"
                        bgColor={pathname.includes(sub.path) ? "blue.600" : "secondary"}
                        textAlign="left"
                        borderStyle={pathname.includes(sub.path) ? "groove" : "dashed"}
                        _focus={{ bg: "btnColor" }}
                        _hover={{
                          bg: "whiteAlpha.200",
                          color: "white",
                        ...
                        {sideBars.map((side, i) =>
                          !pathname.includes(sub.path)
                            ? sub.title === side.title && (
                                <Badge fontSize="10px" key={i} variant="solid" colorScheme="red" mb={1}>
...
const SidebarHeader = () => {
  return (
    <Flex h="150px" flexDirection="column" alignItems="center" gap={1} mt={3} pt={2}>
      <Image boxSize="130px" objectFit="cover" src="/images/elixirdepot.png" alt="etheriumlogo" mt={1} />
...
const SidebarFooter = () => {
  return (
    <Flex h="40px" fontSize="10px" textAlign="center" p={2}>
      © 2024, Elixir Depot Powered by Process Automation (MIS)
    </Flex>
  );
};
